    ...
    <main class="flex flex-column mt-10 pb-6">
        <div class="top items-end">
            <div class="flex flex-column gap-2 content-between">
                ...
                <h2 class="font-size-36">Browse By Category</h2>
            </div>
            <PrimaryButton label="View All" color="primary" size="large" class="self-center mt-6"
...
<style scoped>
main {
    border-bottom: 0.5px #ececec solid;
    padding-bottom: 60px
}
...
.categories-grid {
  ...
  @media (width < 1200px) {
      grid-template-columns: repeat(4, 1fr); /* 4 columns on slightly smaller screens */
  }
  
  @media (width < 900px) {
      grid-template-columns: repeat(3, 1fr); /* 3 columns on tablets and smaller laptops */
  }
  ...
  @media (width < 400px) {
      grid-template-columns: repeat(1, 1fr); /* 1 column on small mobile devices */
  }
}
...
.category-item.active {
  background-color: #e74c3c; /* Red background for the active state */
  color: white;
}
